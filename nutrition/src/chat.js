...
להלן ההקשר המעודכן להיום:`;
  ...
  function buildContext(app) {
    ...
    if (day.sit && (day.sit.breaks || day.sit.longest)) lines.push(`ישיבה בעבודה היום: ${day.sit.breaks || 0} הפסקות קימה, הרצף הארוך ביותר ${day.sit.longest || 0} דק' (תזכורת כל ${day.sit.every || 45} דק').`);
    const syms = app.daySymptoms ? app.daySymptoms(day) : []; if (syms.length) lines.push("איך היא מרגישה היום (סולם 1–5; אנרגיה 5 = הרבה, השאר 5 = חזק): " + syms.map((e) => app.symText(e)).join("; ") + ".");
  ...
  async function parseMeal(app, text, image) {
    ...
    const prompt = `פרקי את תיאור הארוחה הבא לפריטים. השיבי אך ורק במערך JSON של אובייקטים {"name": string, "qty": number, "unit": string, "known": boolean} — בלי טקסט נוסף.
"unit" הוא אחד מ: "גרם", "מ"ל", "יחידה", "כוס", "כף", "כפית", "פרוסה", "מנה", "חופן", "קובייה", "גביע", "קופסה", "פיתה", "כדור".
כללים חשובים:
1. השתמשי בשם מהרשימה (שמות מוכרים למערכת) רק אם הוא מתאר **בדיוק** את מה שנכתב, כולל תיאורים מבחינים: נטול קפאין, חלב צמחי/שיבולת שועל/סויה/שקדים, מלא/לבן, טבעוני, ללא סוכר. אז "known": true.
2. אם אין ברשימה פריט מדויק — אל תבחרי את הקרוב ביותר. כתבי את השם המדויק כפי שתואר (קצר) עם "known": false.
3. משקה או מנה מורכבת פרקי לרכיבים נפרדים בכמויות סבירות (למשל קפה עם חלב וסוכר → קפה, חלב, סוכר כל אחד בשורה משלו).
4. אל תשמיטי פריט ואל תמציאי פריט שלא הוזכר.
הרשימה: ${known.join(", ")}.
דוגמה: "כוס קפה נטול קפאין עם חלב שיבולת שועל וכפית סוכר" → [{"name":"קפה נטול קפאין","qty":1,"unit":"כוס","known":true},{"name":"חלב שיבולת שועל מועשר בסידן","qty":150,"unit":"מ"ל","known":true},{"name":"סוכר","qty":1,"unit":"כפית","known":true}]
דוגמה: "2 פרוסות לחם מלא עם ממרח פיסטוק" → [{"name":"לחם מלא","qty":2,"unit":"פרוסה","known":true},{"name":"ממרח פיסטוק","qty":1,"unit":"כף","known":false}]
