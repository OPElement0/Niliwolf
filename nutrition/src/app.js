...
(function () {
  ...
  function ulGauge(x) {
    ...
    return `<div class="ulg ${lvl}" data-tip="${esc(`${x.he}: ${fmt(x.v)} מתוך גבול ${fmt(lim)} ${x.unit} ליום (${Math.round(r * 100)}%)`)}">
      <svg width="${W}" height="18" viewBox="0 0 ${W} 18" direction="ltr" aria-hidden="true">
        <line x1="${L}" x2="${tp}" y1="${y}" y2="${y}" stroke="var(--good)" stroke-width="4" stroke-linecap="round" opacity=".45"></line>
        <line x1="${tp}" x2="${cp}" y1="${y}" y2="${y}" stroke="var(--warn)" stroke-width="4" opacity=".5"></line>
        <line x1="${cp}" x2="${R}" y1="${y}" y2="${y}" stroke="var(--bad)" stroke-width="4" stroke-linecap="round" opacity=".6"></line>
        <line x1="${R}" x2="${R}" y1="3" y2="15" stroke="var(--bad)" stroke-width="2"></line>
  ...
  function renderToday() {
    ...
    const missing = rest.filter((x) => !isDone(x)).sort((a, b) => (a.status === "over" ? -1 : 0) - (b.status === "over" ? -1 : 0) || a.pct - b.pct);
  ...
  function mealDetail(id) {
    ...
    openModal(`<h2>${esc(m.name)}</h2>
      <p class="ink2 small">${m.time} · ${m.qty} ${esc(m.unit)} · ${m.grams} גרם</p>
