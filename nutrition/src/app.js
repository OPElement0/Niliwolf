...
(function () {
  ...
  function glCardInner(day, isToday) {
    ...
    // symptom reports (▼ at the top; click = edit) — lets nausea/dizziness be read against the curve
    const syms = daySymptoms(day).map((e) => Object.assign({}, e, { t: toMin(e.time) })).filter((e) => e.time && e.t >= x0 && e.t <= x1);
    syms.forEach((e) => { s += `<path class="gl-sym" d="M${X(e.t) - 6} ${top + 1} l12 0 l-6 10 z" data-tip="${esc(`${e.time} · ${symLine(e)}`)}" data-act="sym-edit" data-id="${e.id}" data-date="${App.date}"></path>`; });
  ...
  // ------------------------------------------------------------ symptoms ("איך אני מרגישה")
  // day.symptoms[] = {id, time, energy, nausea, dizziness, hunger, pain: {level, types[], duration, positions[]} | null, note}.
  // A missing level means "not reported in that questionnaire" — deliberately different from a low level.
  const SYM = window.SYMPTOMS, SYM_BY = Object.fromEntries(SYM.map((s) => [s.key, s]));
  const PAIN_HE = { types: Object.fromEntries(window.PAIN_TYPES), duration: Object.fromEntries(window.PAIN_DURATION), positions: Object.fromEntries(window.PAIN_POSITIONS) };
  function daySymptoms(day) { return ((day && day.symptoms) || []).slice().sort((a, b) => (a.time || "").localeCompare(b.time || "")); }
  function symLevel(e, key) { const v = key === "pain" ? (e.pain ? e.pain.level : null) : e[key]; return v == null || v === "" ? null : Number(v); }
  // 0..1 "how unpleasant" — energy is inverted (1 = drained is the worst end).
  function symSeverity(key, lvl) { if (lvl == null) return null; return SYM_BY[key].bad === "low" ? (5 - lvl) / 4 : (lvl - 1) / 4; }
  function symSevClass(key, lvl) { const s = symSeverity(key, lvl); return s == null ? "" : s >= 0.75 ? "sev-4" : s >= 0.5 ? "sev-3" : s >= 0.25 ? "sev-2" : "sev-1"; }
  function painText(p) { if (!p) return ""; const parts = []; if (p.types && p.types.length) parts.push(p.types.map((t) => PAIN_HE.types[t] || t).join(", ")); if (p.duration) parts.push(PAIN_HE.duration[p.duration] || p.duration); if (p.positions && p.positions.length) parts.push("ב" + p.positions.map((t) => PAIN_HE.positions[t] || t).join("/")); return parts.join(" · "); }
  function symParts(e) { return SYM.filter((s) => symLevel(e, s.key) != null).map((s) => ({ key: s.key, he: s.he, lvl: symLevel(e, s.key), detail: s.key === "pain" ? painText(e.pain) : "" })); }
  function symLine(e) { const p = symParts(e).map((x) => `${x.he} ${x.lvl}${x.detail ? ` (${x.detail})` : ""}`); if (e.note && e.note.trim()) p.push(`הערה: ${e.note.trim()}`); return p.join(" · ") || "בלי תסמינים"; }
  function symText(e) { return `${e.time || ""} ${symLine(e)}`.trim(); }
  App.symText = symText; App.daySymptoms = daySymptoms;
  function symChips(e) { return symParts(e).map((x) => `<span class="chip static sym ${symSevClass(x.key, x.lvl)}" title="1 = ${esc(SYM_BY[x.key].low)} · 5 = ${esc(SYM_BY[x.key].high)}">${x.he} <b class="num">${x.lvl}</b>${x.detail ? `<span class="small ink2">· ${esc(x.detail)}</span>` : ""}</span>`).join(""); }
  function symItemHtml(e, dateKey) { return `<div class="sym-item" data-act="sym-edit" data-id="${e.id}" data-date="${dateKey}" title="לחיצה לעריכה או מחיקה"><span class="time num">${esc(e.time || "")}</span><div class="grow"><div class="chips">${symChips(e) || `<span class="small ink2">בלי תסמינים</span>`}</div>${e.note && e.note.trim() ? `<div class="small ink2" style="margin-top:4px">${esc(e.note.trim())}</div>` : ""}</div></div>`; }
  function symTopCardHtml(day, isToday) {
    const list = daySymptoms(day), last = list[list.length - 1];
    return `<div class="card feel"><div class="card-head"><h2>איך אני מרגישה</h2><button class="btn sm primary" data-act="sym-open">+ הוספת תסמינים</button></div>
      ${last ? `<div class="small"><span class="ink2">דיווח אחרון ${esc(last.time || "")}:</span> ${esc(symLine(last))}</div>${list.length > 1 ? `<div class="help" style="margin-top:2px">${list.length} דיווחים ${isToday ? "היום" : "בתאריך זה"} — כולם בתחתית הדף, לפי שעות.</div>` : ""}` : `<p class="help">עדיין לא דיווחת ${isToday ? "היום" : "בתאריך זה"}. אפשר למלא גם בדיעבד — בשאלון בוחרים תאריך ושעה.</p>`}
    </div>`;
  }
  function symListCardHtml(day, isToday, dateKey) {
    const list = daySymptoms(day);
    return `<div class="card" id="sym-list"><div class="card-head"><h2>תסמינים ${isToday ? "היום" : "בתאריך זה"} <span class="small ink2">(${list.length})</span></h2><button class="btn sm" data-act="sym-open">+ הוספה</button></div>
      ${list.length ? `<div class="list">${list.map((e) => symItemHtml(e, dateKey)).join("")}</div><p class="help" style="margin-top:6px">לחיצה על דיווח: עריכה או מחיקה. הכול נכנס למעקב לאורך זמן ולדוח.</p>` : `<p class="help">אין דיווחים ${isToday ? "היום" : "בתאריך זה"}.</p>`}
    </div>`;
  }
  // questionnaire modal — draft in window._editSym = {date, origDate, isNew, entry}
  function symOpen(entry, dateKey) {
    const e = entry ? JSON.parse(JSON.stringify(entry)) : { id: uid("sy"), time: nowTime(), note: "" };
    e.pain = e.pain || null;
    window._editSym = { date: dateKey || App.date, origDate: dateKey || App.date, isNew: !entry, entry: e };
    symModal();
  }
  function symDraftFromDom() {
    const d = window._editSym; if (!d) return;
    if ($("sym-date") && $("sym-date").value) d.date = $("sym-date").value;
    if ($("sym-time") && $("sym-time").value) d.entry.time = $("sym-time").value;
    if ($("sym-note")) d.entry.note = $("sym-note").value;
  }
  function symModal() {
    const d = window._editSym, e = d.entry;
    const prev = document.querySelector(".modal"), scrollTop = prev ? prev.scrollTop : 0;
    const scale = (key) => { const cur = symLevel(e, key), m = SYM_BY[key]; return `<div><div class="sym-scale" role="group" aria-label="${esc(m.he)}">${[1, 2, 3, 4, 5].map((v) => `<button type="button" class="${cur === v ? "on" : ""}" data-act="sym-level" data-key="${key}" data-v="${v}" aria-pressed="${cur === v}">${v}</button>`).join("")}</div><div class="sym-ends"><span>${esc(m.low)}</span><span>${esc(m.high)}</span></div></div>`; };
    const multi = (k, opts, arr) => `<div class="chips">${opts.map(([v, he]) => `<button type="button" class="chip ${(arr || []).includes(v) ? "on" : ""}" data-act="sym-pain-multi" data-k="${k}" data-v="${v}">${esc(he)}</button>`).join("")}</div>`;
    const single = (opts, cur) => `<div class="chips">${opts.map(([v, he]) => `<button type="button" class="chip ${cur === v ? "on" : ""}" data-act="sym-pain-single" data-v="${v}">${esc(he)}</button>`).join("")}</div>`;
    const painSub = e.pain && e.pain.level ? `<div class="sym-sub">
        <div class="row" style="gap:6px"><span class="lbl">סוג</span>${multi("types", window.PAIN_TYPES, e.pain.types)}</div>
        <div class="row" style="gap:6px"><span class="lbl">אורך</span>${single(window.PAIN_DURATION, e.pain.duration)}</div>
        <div class="row" style="gap:6px"><span class="lbl">פוזיציה</span>${multi("positions", window.PAIN_POSITIONS, e.pain.positions)}</div>
      </div>` : "";
    openModal(`<h2>${d.isNew ? "הוספת תסמינים" : "עריכת דיווח"}</h2>
      <p class="help">סמני רק מה שרלוונטי — שורה שלא נגעת בה לא נרשמת. לחיצה חוזרת על מספר מבטלת אותו.</p>
      <div class="inline-form" style="margin-top:8px"><div class="field short"><label for="sym-date">תאריך</label><input id="sym-date" type="date" value="${esc(d.date)}" style="min-height:36px;padding:4px 8px"></div><div class="field short"><label for="sym-time">שעה</label><input id="sym-time" type="time" value="${esc(e.time || nowTime())}" style="min-height:36px;padding:4px 8px"></div></div>
      <div style="margin-top:8px">${SYM.map((m) => `<div class="sym-row"><div><b>${esc(m.he)}</b>${m.key === "pain" && e.pain && e.pain.level ? `<div class="help">${esc(painText(e.pain) || "פרטי למטה: סוג, אורך, פוזיציה")}</div>` : ""}</div>${scale(m.key)}${m.key === "pain" ? painSub : ""}</div>`).join("")}</div>
      <div class="field" style="margin-top:10px"><label for="sym-note">הערה חופשית</label><textarea id="sym-note" placeholder="מה קדם לזה, מה עזר, משהו חריג…">${esc(e.note || "")}</textarea></div>
      <div class="actions">${d.isNew ? "" : `<button class="btn danger" data-act="sym-del">מחיקה</button>`}<button class="btn" data-act="modal-close">ביטול</button><button class="btn primary" data-act="sym-save">שמירה</button></div>`);
    const cur = document.querySelector(".modal"); if (cur && scrollTop) cur.scrollTop = scrollTop;
  }
  function symSave() {
    const d = window._editSym; if (!d) return; symDraftFromDom();
    const e = d.entry;
    if (e.pain && !e.pain.level) e.pain = null;
    if (e.pain) { e.pain.types = e.pain.types || []; e.pain.positions = e.pain.positions || []; if (!e.pain.duration) delete e.pain.duration; }
    e.note = (e.note || "").trim(); e.time = e.time || nowTime();
    if (!SYM.some((s) => symLevel(e, s.key) != null) && !e.note) { toast("סמני לפחות תסמין אחד או כתבי הערה"); return; }
    const key = d.date || App.date;
    if (d.origDate !== key) { const od = getDay(d.origDate); od.symptoms = (od.symptoms || []).filter((x) => x.id !== e.id); saveDay(d.origDate); }
    const day = getDay(key); day.symptoms = day.symptoms || [];
    const i = day.symptoms.findIndex((x) => x.id === e.id); if (i >= 0) day.symptoms[i] = e; else day.symptoms.push(e);
    day.symptoms.sort((a, b) => (a.time || "").localeCompare(b.time || ""));
    saveDay(key); window._editSym = null; closeModal(); render();
    toast(key === App.date ? (d.isNew ? "הדיווח נרשם" : "הדיווח עודכן") : `נרשם לתאריך ${fmtDate(key)}`);
  }
  function symDelete() {
    const d = window._editSym; if (!d || d.isNew) { closeModal(); return; }
    if (!confirm("למחוק את הדיווח?")) return;
    const od = getDay(d.origDate); od.symptoms = (od.symptoms || []).filter((x) => x.id !== d.entry.id); saveDay(d.origDate);
    window._editSym = null; closeModal(); render(); toast("הדיווח נמחק");
  }
  function symDayModal(key) {
    const list = daySymptoms(App.state.days[key]);
    openModal(`<h2>תסמינים · ${esc(fmtDateLong(key))}</h2>${list.length ? `<div class="list">${list.map((e) => symItemHtml(e, key)).join("")}</div>` : `<p class="help">אין דיווחים בתאריך זה.</p>`}<div class="actions"><button class="btn" data-act="sym-open" data-date="${key}">+ הוספה לתאריך זה</button><button class="btn" data-act="modal-close">סגירה</button></div>`);
  }
  // per-day aggregate for history/report: {key: {n, mean, worst, sev}}; worst = the unpleasant end of the day.
  function symDayStats(day) {
    const list = daySymptoms(day), out = {};
    SYM.forEach((s) => { const v = list.map((e) => symLevel(e, s.key)).filter((x) => x != null); if (!v.length) return; const worst = s.bad === "low" ? Math.min(...v) : Math.max(...v); out[s.key] = { n: v.length, mean: v.reduce((a, b) => a + b, 0) / v.length, worst, sev: symSeverity(s.key, worst) }; });
    return out;
  }
  ...
  const REP_FIELDS = [
    ...
    ["notes", "הערות יומיות"], ["symptoms", "תסמינים (איך הרגשתי)"], ["averages", "ממוצעים לתקופה מול היעדים"], ["sources", "מקורות עיקריים לכל רכיב מרכזי"], ["accuracy", "הערת דיוק — מקור הערכים"],
  ];
  const REP_PRESETS = {
    claude: { fields: { preg: 1, targets: 1, daily: 1, meals: 1, mealNut: 1, supps: 1, activity: 1, gl: 1, labs: 1, notes: 1, symptoms: 1, averages: 1, sources: 1, accuracy: 1 }, nutMode: "all", name: "" },
    nurse: { fields: { preg: 1, targets: 1, daily: 1, meals: 0, mealNut: 0, supps: 1, activity: 1, gl: 0, labs: 1, notes: 0, symptoms: 1, averages: 1, sources: 1, accuracy: 1 }, nutMode: "main", name: "" },
  };
  ...
  function reportModal(opts) {
    ...
    const logged = days.filter((k) => App.state.days[k] && ((App.state.days[k].meals || []).length || (App.state.days[k].walks || []).length || (App.state.days[k].symptoms || []).length || App.state.days[k].weight_kg));
  ...
  function reportData() {
    ...
    days.forEach((k) => {
      ...
      const row = { k, hasMeals, day, tot: null, food: null, fromSupp: null, targets: null, supp: [], walks: dayWalks(day), sun: (day && day.sun) || [], weight: day && day.weight_kg, notes: day && day.notes, glucose: (day && day.glucose) || [], symptoms: daySymptoms(day), gl: null, week: null, nMeals: hasMeals ? day.meals.length : 0, full: false, retinol: 0, top: {} };
  ...
  function reportText(d) {
    ...
    if (f.symptoms) {
      const sd = d.rows.filter((x) => x.symptoms.length);
      L.push("## תסמינים — איך הרגשתי (סולם 1–5; אנרגיה: 1 = מרוקנת, 5 = מלאת אנרגיה; השאר: 1 = קל, 5 = חזק)");
      if (!sd.length) L.push("לא דווחו תסמינים בטווח.");
      sd.forEach((x) => { L.push(`### ${repDate(x.k)}`); x.symptoms.forEach((e) => L.push(`- ${e.time || ""}: ${symLine(e)}`)); });
      if (sd.length) {
        L.push(""); L.push("| תסמין | דיווחים | ימים | ממוצע | הכי חמור |"); L.push("|---|---|---|---|---|");
        SYM.forEach((s) => { const vals = sd.flatMap((x) => x.symptoms.map((e) => symLevel(e, s.key)).filter((v) => v != null)); if (!vals.length) return; const nd = sd.filter((x) => x.symptoms.some((e) => symLevel(e, s.key) != null)).length; L.push(`| ${s.he} | ${vals.length} | ${nd} | ${fmt(vals.reduce((a, b) => a + b, 0) / vals.length, 1)} | ${s.bad === "low" ? Math.min(...vals) : Math.max(...vals)} |`); });
      }
      L.push("");
    }
...
<div class="foot">הופק מכלי מעקב תזונה אישי. ערכי מזון משוערים; יעדים לפי DRI להריון (NIH/IOM) מותאמים אישית. אינו תחליף לייעוץ רפואי.</div>
  ...
  function reportCsv(d) {
    ...
    const rows = [["תאריך", "שבוע הריון", "ארוחות", "יום מלא", ...keys.map((n) => `${NUT_BY[n].he} ממזון (${NUT_BY[n].unit})`), ...d.suppKeys.map((n) => `${NUT_BY[n].he} מתוספים`), ...keys.map((n) => `יעד ${NUT_BY[n].he}`), "תוספים שסומנו", "הליכה (דק')", "משקל", ...SYM.map((s) => `${s.he} (${s.bad === "low" ? "הכי נמוך" : "הכי גבוה"} ביום)`), "דיווחי תסמינים"]];
    d.rows.forEach((x) => { rows.push([x.k, x.week != null ? x.week : "", x.nMeals, x.hasMeals ? (x.full ? "כן" : "חלקי") : "אין רישום", ...keys.map((n) => x.hasMeals ? Math.round((x.food[n] || 0) * 10) / 10 : ""), ...d.suppKeys.map((n) => x.day ? Math.round((x.fromSupp[n] || 0) * 100) / 100 : ""), ...keys.map((n) => x.targets && x.targets[n] != null ? x.targets[n] : ""), x.day ? x.supp.map((s) => `${s.name} ${s.taken}/${s.doses}`).join("; ") : "", x.walks.reduce((a, w) => a + w.minutes, 0) || "", x.weight || "", ...SYM.map((s) => { const st = symDayStats(x.day)[s.key]; return st ? st.worst : ""; }), x.symptoms.length || ""]); });
  ...
  function renderToday() {
    ...
    $("panel-today").innerHTML = `
      <div class="stack">
        ...
        ${symListCardHtml(day, isToday, App.date)}
      </div>`;
  }
  ...
  // ---- History
  let histRange = 30, histKey = "protein", histLab = "ferritin", histSym = "nausea";
  function renderHistory() {
    const todayK = dateKeyOf(new Date());
    const allKeys = Object.keys(App.state.days).filter((k) => (App.state.days[k].meals || []).length || (App.state.days[k].symptoms || []).length).sort();
    ...
    // symptoms ("איך אני מרגישה"): heatmap of the day's worst level, trend per symptom, what repeats in abdominal pain
    const symStats = Object.fromEntries(keys.map((k) => [k, symDayStats(App.state.days[k])]));
    const symDays = keys.filter((k) => Object.keys(symStats[k]).length);
    let symHtml = "";
    if (symDays.length) {
      const dayAttrs = (k) => `data-act="sym-day" data-date="${k}"`;
      const heatS = window.Charts.heatmap({ rows: SYM.map((s) => s.he), cols: keys.map((k) => ({ label: n <= 31 ? fmtDate(k) : (parseKey(k).getDay() === 0 ? fmtDate(k) : "") })), cells: SYM.map((s) => keys.map((k) => { const st = symStats[k][s.key]; if (!st) return Object.keys(symStats[k]).length ? { p: null, tip: `${fmtDate(k)} · ${s.he}: לא דווח · לחיצה לפרטי היום`, attrs: dayAttrs(k) } : null; return { p: 0.13 + st.sev * 0.87, tip: `${fmtDate(k)} · ${s.he}: ${s.bad === "low" ? "הכי נמוך" : "הכי גבוה"} ${st.worst}, ממוצע ${fmt(st.mean, 1)} (${st.n} ${st.n === 1 ? "דיווח" : "דיווחים"}) · לחיצה לפרטי היום`, attrs: dayAttrs(k) }; })) });
      const sm = SYM_BY[histSym] || SYM[0];
      const sp = keys.map((k) => { const st = symStats[k][sm.key]; return { label: fmtDate(k), y: st ? Math.round(st.mean * 10) / 10 : null }; });
      const sAvg = sp.map((p, i) => { const win = sp.slice(Math.max(0, i - 6), i + 1).map((q) => q.y).filter((v) => v != null); return win.length >= 3 ? win.reduce((a, b) => a + b, 0) / win.length : null; });
      const symTrendSvg = window.Charts.line({ points: sp, avg: sAvg, yMin: 1, yMax: 5, unit: "", yLabel: sm.he, height: 180 });
      const summary = SYM.map((s) => { const ds = symDays.filter((k) => symStats[k][s.key]); if (!ds.length) return `<div><span>${s.he}</span><span class="ink2">לא דווח</span></div>`; const all = ds.flatMap((k) => daySymptoms(App.state.days[k]).map((e) => symLevel(e, s.key)).filter((v) => v != null)); const worst = s.bad === "low" ? Math.min(...all) : Math.max(...all); return `<div><span>${s.he}</span><span class="num">${ds.length} ימים · ממוצע ${fmt(all.reduce((a, b) => a + b, 0) / all.length, 1)} · ${s.bad === "low" ? "הכי נמוך" : "הכי גבוה"} ${worst}</span></div>`; }).join("");
      const pains = symDays.flatMap((k) => daySymptoms(App.state.days[k]).filter((e) => e.pain && e.pain.level).map((e) => Object.assign({ k }, e)));
      const countChips = (arr, he) => { const c = {}; arr.forEach((v) => { c[v] = (c[v] || 0) + 1; }); const ent = Object.entries(c).sort((a, b) => b[1] - a[1]); return ent.length ? ent.map(([v, cnt]) => `<span class="chip static">${esc(he[v] || v)} <b class="num">${cnt}</b></span>`).join("") : `<span class="help">לא צוין</span>`; };
      const hourBucket = (t) => { const h = toMin(t) / 60; return h < 6 || h >= 22 ? "לילה (22–06)" : h < 12 ? "בוקר (06–12)" : h < 17 ? "צהריים (12–17)" : "ערב (17–22)"; };
      const painHtml = pains.length ? `<div class="stack" style="gap:8px">
          <div class="small">${pains.length} ${pains.length === 1 ? "אפיזודה" : "אפיזודות"} ב-${new Set(pains.map((e) => e.k)).size} ימים · עוצמה ממוצעת <b class="num">${fmt(pains.reduce((a, e) => a + Number(e.pain.level), 0) / pains.length, 1)}</b> · הכי גבוה <b class="num">${Math.max(...pains.map((e) => Number(e.pain.level)))}</b></div>
          <div class="row" style="gap:6px"><span class="small ink2" style="min-width:56px">סוג</span><span class="chips">${countChips(pains.flatMap((e) => e.pain.types || []), PAIN_HE.types)}</span></div>
          <div class="row" style="gap:6px"><span class="small ink2" style="min-width:56px">אורך</span><span class="chips">${countChips(pains.map((e) => e.pain.duration).filter(Boolean), PAIN_HE.duration)}</span></div>
          <div class="row" style="gap:6px"><span class="small ink2" style="min-width:56px">פוזיציה</span><span class="chips">${countChips(pains.flatMap((e) => e.pain.positions || []), PAIN_HE.positions)}</span></div>
          <div class="row" style="gap:6px"><span class="small ink2" style="min-width:56px">שעות</span><span class="chips">${countChips(pains.filter((e) => e.time).map((e) => hourBucket(e.time)), {})}</span></div>
        </div>` : `<p class="help">לא דווחו כאבי בטן בטווח.</p>`;
      symHtml = `<div class="card"><div class="card-head"><h3>תסמינים — איך הרגשתי לאורך זמן</h3><span class="help">${symDays.length} ימים עם דיווח מתוך ${n}</span></div>
        <div class="chart">${heatS}</div>
        <div class="legend"><span><i style="background:var(--seq-1)"></i>1</span><span><i style="background:var(--seq-2)"></i>2</span><span><i style="background:var(--seq-3)"></i>3</span><span><i style="background:var(--seq-4)"></i>4</span><span><i style="background:var(--seq-5)"></i>5 (הכי חמור ביום)</span><span><i style="background:var(--surface-2)"></i>לא דווח</span></div>
        <p class="help" style="margin-top:4px">באנרגיה הצבע הפוך: כהה = מרוקנת. לחיצה על תא פותחת את הדיווחים של אותו יום.</p>
        <div class="mini-list" style="margin-top:10px">${summary}</div>
        <div class="grid equal" style="margin-top:12px">
          <div><div class="card-head"><h3>מגמה</h3><select id="hist-sym" data-act="hist-sym" style="width:auto;min-height:34px;padding:4px 8px">${SYM.map((s) => `<option value="${s.key}" ${s.key === sm.key ? "selected" : ""}>${s.he}</option>`).join("")}</select></div><div class="chart">${symTrendSvg}</div><div class="legend"><span><i style="background:var(--accent)"></i>ממוצע יומי</span><span><i style="background:var(--accent-2)"></i>ממוצע נע 7 ימים</span></div></div>
          <div><div class="card-head"><h3>כאבי בטן — מה חוזר</h3></div>${painHtml}</div>
        </div>
      </div>`;
    } else symHtml = `<div class="card"><div class="card-head"><h3>תסמינים — איך הרגשתי לאורך זמן</h3></div><p class="help">אין דיווחי תסמינים בטווח. מדווחים מכרטיס "איך אני מרגישה" בראש טאב היום.</p></div>`;
    // summary tiles
  ...
  const actions = {
    ...
    // symptoms
    "sym-open": (el) => symOpen(null, el.dataset.date || App.date),
    "sym-edit": (el) => { const day = App.state.days[el.dataset.date]; const e = day && (day.symptoms || []).find((x) => x.id === el.dataset.id); if (e) symOpen(e, el.dataset.date); },
    "sym-level": (el) => { symDraftFromDom(); const e = window._editSym.entry, k = el.dataset.key, v = Number(el.dataset.v); const nv = symLevel(e, k) === v ? null : v; if (k === "pain") e.pain = nv == null ? null : Object.assign({ types: [], positions: [] }, e.pain || {}, { level: nv }); else if (nv == null) delete e[k]; else e[k] = nv; symModal(); },
    "sym-pain-multi": (el) => { symDraftFromDom(); const e = window._editSym.entry; if (!e.pain) return; const arr = e.pain[el.dataset.k] = e.pain[el.dataset.k] || []; const i = arr.indexOf(el.dataset.v); if (i >= 0) arr.splice(i, 1); else arr.push(el.dataset.v); symModal(); },
    "sym-pain-single": (el) => { symDraftFromDom(); const e = window._editSym.entry; if (!e.pain) return; e.pain.duration = e.pain.duration === el.dataset.v ? null : el.dataset.v; symModal(); },
    "sym-save": () => symSave(),
    "sym-del": () => symDelete(),
    "sym-day": (el) => symDayModal(el.dataset.date),
    // history
  ...
  document.addEventListener("change", (e) => {
    ...
    if (act === "hist-sym") { histSym = el.value; renderHistory(); }
